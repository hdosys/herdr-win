...
Local startup and compatibility failures show their reason and restart guidance without blocking a healthy saved machine. A previously saved machine selection is still restored normally; a Local failure does not select a different machine.
...
The selected remote executable must also advertise `remote_connect_only`, so reconnects and readiness checks cannot start a missing server. Standalone setup can update an executable that lacks this capability; background reconnect cannot.

A version difference alone does not require a server restart. Replacing a Windows remote payload still requires approval to stop its server and release runtime leases. Background reconnect never performs that replacement.
