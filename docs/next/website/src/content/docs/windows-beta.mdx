...
Install native Windows builds by downloading the newest `herdr-win_v<version>_windows_amd64_setup.exe` from the [herdr-win releases](https://github.com/hdosys/herdr-win/releases). The per-user NSIS setup requires no administrator access, registers **Herdr Win** in Installed Apps, and puts `%LOCALAPPDATA%\Programs\Herdr\bin` on the user `PATH`. One stable launcher selects immutable runtimes under `%LOCALAPPDATA%\Programs\Herdr\runtime` so active sessions can finish before a newer runtime is activated.
...
Every supported client can attach to x86_64 and ARM64 Windows hosts. Herdr uses an exact version- and protocol-matching binary from `PATH` or the stable `%USERPROFILE%\.herdr\remote\herdr.exe` payload. Explicit provisioning transfers and validates the complete portable ZIP without running the managed installer or changing remote `PATH`; ordinary non-interactive attach never installs. The Windows OpenSSH default shell must be `cmd.exe` or PowerShell 7, and the server starts in the SSH user's existing active desktop session so it survives SSH closure. Windows PowerShell 5.1, an unknown shell, or an ambiguous desktop session fails closed. Windows remote hosts do not support live handoff.
