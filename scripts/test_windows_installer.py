...
BUILD_VALIDATOR = NSI.parent / "validate-build.ps1"
...
MAIN = PROJECT_ROOT / "src/main.rs"
SESSION = PROJECT_ROOT / "src/session.rs"
...
class WindowsInstallerStaticTests(unittest.TestCase):
    ...
    def test_native_helper_owns_exact_layout_and_lifecycle_contracts(self) -> None:
        ...
        self.assertIn(
            'INSTALLER_STOP_SESSIONS_COMMAND: &str = "__installer-stop-sessions"',
            managed,
        )
        self.assertEqual(
            (managed + main + lifecycle).count('"__installer-stop-sessions"'), 1
        )
        self.assertIn("installer_stop_sessions_requested", main)
        self.assertIn("stop_all_sessions_for_uninstall", main)
        self.assertIn("stop_all_sessions_for_uninstall", session)
        self.assertIn("INSTALLER_STOP_WAIT_TIMEOUT", session)
        ...
        for contract in (
            ...
            "stop_installed_sessions",
            "INSTALLER_STOP_SESSIONS_COMMAND",
            "INSTALLER_STOP_TIMEOUT",
        ):
        ...
        uninstall_layout = lifecycle[lifecycle.index("fn uninstall_layout") :]
        self.assertLess(
            uninstall_layout.index("validate_managed_root"),
            uninstall_layout.index("stop_installed_sessions"),
        )
        self.assertLess(
            uninstall_layout.index("stop_installed_sessions"),
            uninstall_layout.index("acquire_coordination"),
        )
        self.assertLess(
            uninstall_layout.index("acquire_coordination"),
            uninstall_layout.index("uninstall.pending"),
        )
        stop_sessions = lifecycle[
            lifecycle.index("fn stop_installed_sessions") : lifecycle.index(
                "fn stage_managed_directory_for_uninstall"
            )
        ]
        self.assertIn("read_required_active_pointer", stop_sessions)
        self.assertIn("payload_path", stop_sessions)
        self.assertIn("open_shared_lease", stop_sessions)
        self.assertIn("configure_payload_child", stop_sessions)
        self.assertIn("installer_stop_retry_message", stop_sessions)
        self.assertIn(
            "Close all managed Herdr sessions and retry uninstall.", lifecycle
        )
        self.assertNotIn('join("bin").join("herdr.exe")', stop_sessions)
    ...
    def test_powershell_is_build_time_only_and_never_a_runtime_payload(self) -> None:
        combined = "\n".join(
            text(path)
            for path in (NSI, HELPER_CLI, HELPER_LIFECYCLE, HELPER_REGISTRY)
        )
        ...
        self.assertIn('"packaging\\windows\\installer\\validate-build.ps1"', packager)
        self.assertIn("Invoke-NativeChecked powershell.exe", packager)
        self.assertLess(
            packager.index("Invoke-NativeChecked powershell.exe"),
            packager.index('Invoke-NativeChecked $makensis'),
        )
        validator = text(BUILD_VALIDATOR)
        self.assertIn("Set-StrictMode -Version 2.0", validator)
        self.assertIn("Assert-SafeWindowsLeaf", validator)
        self.assertIn("Assert-Bitmap", validator)
        self.assertNotIn("HashData", validator)
    ...
    def test_nsis_uses_the_native_helper_and_preserves_presentation(self) -> None:
        ...
        self.assertIn("ManifestSupportedOS all", nsi)
        self.assertIn('GetProcAddress(p r0, m "GetDpiForWindow")', nsi)
        self.assertIn("APP_EXIT_INVALID_ARGUMENTS 30", nsi)
        self.assertIn("APP_EXIT_UNSUPPORTED_PLATFORM 50", nsi)
        self.assertIn("APP_EXIT_INSTALL_FAILED 70", nsi)
        self.assertIn("APP_EXIT_UNINSTALL_FAILED 80", nsi)
        self.assertIn("SetErrorLevel $FailureExitCode", nsi)
        self.assertIn('StrCmp $0 "/WINGET" installer_winget_arguments', nsi)
        self.assertIn('StrCmp $0 "/REMOVE_SETTINGS" un_arguments_settings', nsi)
        self.assertIn('StrCmp $0 "/REMOVE_SKILL" un_arguments_skill', nsi)
        self.assertIn("Unsupported setup arguments", nsi)
        self.assertIn("Unsupported uninstall arguments", nsi)
        self.assertIn("MUI_CUSTOMFUNCTION_ABORT PreventInstallMutationAbort", nsi)
        self.assertIn(
            "MUI_CUSTOMFUNCTION_UNABORT un.PreventUninstallMutationAbort", nsi
        )
        self.assertIn('StrCpy $InstallMutationActive "1"', nsi)
        self.assertIn("Call DisableInstallCancellation", nsi)
        self.assertIn("Call un.DisableUninstallCancellation", nsi)
        self.assertLess(
            nsi.index('StrCpy $InstallMutationActive "1"', nsi.index("Section")),
            nsi.index('"$PLUGINSDIR\\installer-helper.exe" install'),
        )
        self.assertLess(
            nsi.index(
                'StrCpy $InstallMutationActive "1"', nsi.index('Section "Uninstall"')
            ),
            nsi.index('"$PLUGINSDIR\\installer-helper.exe" uninstall'),
        )
        self.assertIn("SendMessageTimeoutW", nsi)
        self.assertIn(
            "Uninstall stops running managed sessions, then removes the managed program, user PATH entry, and Windows Installed Apps registration",
            nsi,
        )
    ...
    def test_packager_validates_three_distinct_x64_binaries(self) -> None:
        ...
        runtime_script_mentions = [
            line
            for line in packager.splitlines()
            if ".ps1" in line and "validate-build.ps1" not in line
        ]
        self.assertEqual(runtime_script_mentions, [])
        ...
        self.assertIn(
            'EnvironmentVariables.Remove("HERDR_REMOTE_SIDECAR_V1")', packager
        )
    ...
    def test_real_fault_matrix_covers_retries_and_pending_activation(self) -> None:
        ...
        for contract in (
            ...
            "Exact setup argument rejection passed",
            "Exact uninstall argument rejection and running-server shutdown passed",
            ...
            "HERDR_REMOTE_SIDECAR_V1",
            'Start-Process -FilePath $managedLauncher -ArgumentList @("server")',
            '"_?=$installRoot"',
            '"state\\pending"',
            '"runtime\\$BuildId"',
            '"quiet-uninstall"',
        ...
        self.assertIn("Get-TestFileSha256", fault)
        self.assertNotIn("Get-FileHash", fault)
    ...
    def test_native_helper_owns_terminal_quiet_uninstall(self) -> None:
        ...
        self.assertIn(
            "QUIET_UNINSTALL_OPERATION_TIMEOUT: Duration = Duration::from_secs(25)",
            lifecycle,
        )
        self.assertIn(
            "QUIET_UNINSTALL_CLEANUP_TIMEOUT: Duration = Duration::from_secs(5)",
            lifecycle,
        )
        self.assertIn("terminate_and_reap", lifecycle)
        self.assertIn("wait_for_empty", lifecycle)
        self.assertIn("QueryInformationJobObject", lifecycle)
        self.assertIn("ActiveProcesses", lifecycle)
        self.assertIn(
            "quiet-uninstall process tree remained active after terminal result",
            lifecycle,
        )
        self.assertIn("30 second total deadline", lifecycle)
        self.assertIn("ProcessJob::new", lifecycle)
        self.assertIn("delete_running_executable", lifecycle)
        self.assertIn("FileDispositionInfoEx", lifecycle)
        self.assertIn("FILE_DISPOSITION_FLAG_POSIX_SEMANTICS", lifecycle)
        self.assertLess(
            lifecycle.index("delete_running_executable(&quiet.moved_helper_path)"),
            lifecycle.index("quiet.publish_result(success)"),
        )
        self.assertNotIn("wait_for_runner_and_remove_helper", lifecycle)
        ...
        self.assertIn("remove_raw_owned_path_entries", registry)
        self.assertIn("replace('/', \"\\\\\")", registry)
        self.assertIn("trim_end_matches('\\\\')", registry)
